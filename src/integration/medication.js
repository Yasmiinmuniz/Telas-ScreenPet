...
form.addEventListener("submit", async function (event) {
  ...
  const url = "http://localhost:8097/api/v5/cadastrar-medicamento";
...
function carregarMedicamentos() {
  fetch('http://localhost:8097/api/v5/listar-medicamentos')
...
document.addEventListener('DOMContentLoaded', carregarMedicamentos);
    ...
    function carregarDadosDoMedicamentoParaEdicao(medicamentoId) {
        ...
        fetch(`http://localhost:8097/api/v5/pesquisar-medicamento/${medicamentoId}`)
            .then(response => response.json())
            .then(data => {
    ...
    // Adicionar um ouvinte de eventos para o botão de edição
    const editarMedicamentoButtons = document.querySelectorAll('.btn-editar-medicamento');
    editarMedicamentoButtons.forEach(button => {
        button.addEventListener('click', function(event) {
            const medicamentoId = event.target.getAttribute('data-id');
            carregarDadosDoMedicamentoParaEdicao(medicamentoId);
        });
    });

    document.getElementById('save-edited-medicamento').addEventListener('click', function() {
        // Aqui você deve coletar os dados do formulário de edição
    });